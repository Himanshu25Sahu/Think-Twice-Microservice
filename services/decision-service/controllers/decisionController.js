...
export const createDecision = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('Failed to emit decision:created event:', error.message);
...
export const updateDecision = async (req, res) => {
  try {
    ...
    // Emit event for analytics service
    try {
      await emitEvent(STREAM_NAMES.DECISION_UPDATED, {
        userId: req.user._id.toString(),
        decision: updatedDecision,
        userEmail: req.user.email,
      });
    } catch (error) {
      console.error('Failed to emit decision:updated event:', error.message);
      // Don't fail the request if event emission fails
    }
